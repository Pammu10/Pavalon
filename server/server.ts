...
import { GameState, Player, GamePhase, Role, Alignment, Quest, Message, ClientToServerEvents, ServerToClientEvents, User } from './types';
...
app.post('/api/login', async (req: Request, res: Response) => {
    ...
    try {
        const userRow = await db.get<User>('SELECT id, username, password_hash FROM users WHERE username = ?', [username]);
        if (!userRow) {
            return res.status(401).json({ message: 'Invalid credentials.' });
        }
        const match = await bcrypt.compare(password, (userRow as any).password_hash);
        if (!match) {
            return res.status(401).json({ message: 'Invalid credentials.' });
        }
...
class GameService {
    ...
    findGameByReconnectingUserId(userId: number): [string, GameState] | undefined {
        for (const [code, state] of this.games.entries()) {
    ...
    handleJoinRoom(socket: Socket, user: User, roomCode?: string) {
        ...
        const newPlayer: Player = { id: socket.id, userId: user.id, name: user.username, role: null, alignment: null, isHost, hasVoted: false, status: 'CONNECTED' };
        gameState.players.push(newPlayer);

        this.io.to(code).emit('updateGameState', gameState);
    }

    handleReconnect(socket: any, user: User, [roomCode, gameState]: [string, GameState]) {
    ...
    async getPlayerStats(userId: number) {
        ...
        const totalGames = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM player_performance WHERE user_id = ?', [userId]);
        const wins = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM player_performance WHERE user_id = ? AND won = 1', [userId]);
        const goodGames = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM player_performance WHERE user_id = ? AND alignment = ?', [userId, Alignment.GOOD]);
        const goodWins = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM player_performance WHERE user_id = ? AND alignment = ? AND won = 1', [userId, Alignment.GOOD]);
        const evilGames = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM player_performance WHERE user_id = ? AND alignment = ?', [userId, Alignment.EVIL]);
        const evilWins = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM player_performance WHERE user_id = ? AND alignment = ? AND won = 1', [userId, Alignment.EVIL]);
        const recentMatches = await db.all<any[]>('SELECT m.winner, pp.role, pp.won, m.played_at FROM matches m JOIN player_performance pp ON m.id = pp.match_id WHERE pp.user_id = ? ORDER BY m.played_at DESC LIMIT 10', [userId]);
        ...
        return {
            ...
            winRate: totalGames?.count ? Math.round((wins!.count / totalGames.count) * 100) : 0,
            goodWinRate: goodGames?.count ? Math.round((goodWins!.count / goodGames.count) * 100) : 0,
            evilWinRate: evilGames?.count ? Math.round((evilWins!.count / evilGames.count) * 100) : 0,
    ...
    async endGame(gameState: GameState, winner: Alignment | null, reason: string) {
        ...
        if(winner) { // Only record stats for games with a clear winner
             try {
                const matchResult = await db.run('INSERT INTO matches (winner) VALUES (?)', [winner]);
                ...
                for(const player of gameState.players) {
                    if(player.role && player.alignment && player.userId) {
                         await db.run(
                            ...
                            [player.userId, matchId, player.role, player.alignment, player.alignment === winner]
                        );
                    }
                }
            } catch(e) {
    ...
    handleDisconnect(playerId: string) {
        ...
        if (gameState.phase === GamePhase.LOBBY || gameState.phase === GamePhase.END_GAME) {
        ...
        this.io.to(roomCode).emit('updateGameState', gameState);
        console.log(`Player ${disconnectedPlayer.name} disconnected from ${roomCode}. Starting ${RECONNECT_TIMEOUT / 1000}s timer.`);
    }
    ...
    private processTeamVote(gameState: GameState) {
        ...
        if (approvals > connectedPlayers.length / 2) { // Team approved
