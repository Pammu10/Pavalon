...
const FinalRosterDisplay: React.FC<{ finalRoles: Role[] }> = ({
  finalRoles,
}) => {
  const goodTeam = finalRoles.filter(
    (r) => ROLES[r].alignment === Alignment.GOOD
  );
  const evilTeam = finalRoles.filter(
  ...
  const TeamPanel: React.FC<{
    ...
    return (
      <div
        style={noisyBgStyle}
        className={`p-4 rounded-xl border-2 ${classes.border} shadow-lg`}
      >
        <h3
          className={`font-eaglelake text-2xl font-bold ${classes.text} flex items-center gap-3 mb-2`}
        >
          <span
            className={`w-3 h-3 rounded-full ${classes.dot} shadow-md`}
          ></span>
          {title}
        </h3>
  ...
  return (
    <>
      ...
      <div className="mt-6 text-center">
        <Button onClick={onStart} disabled={!validation.isValid || isPaused}>
          Start Game
        </Button>
        {!validation.isValid && (
...
const GameTips: React.FC = () => {
  ...
  return (
    ...
    >
      <h4 className="font-bold text-yellow-400 mb-2 font-eaglelake">
        Game Tip
      </h4>
        ...
        <header className="flex flex-col sm:flex-row justify-between items-center mb-6 text-center sm:text-left gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button
              variant="danger"
              onClick={leaveRoom}
              className="text-sm py-1.5 px-4 flex-1 flex items-center gap-2 justify-center"
            >
              <LogOut size={16} />
              Leave
            </Button>
