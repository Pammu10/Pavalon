  ...
  <div className="flex items-center justify-between bg-slate-800/50 p-3 rounded-lg">
    ...
    <button
      ...
      className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${
        enabled ? "bg-yellow-600" : "bg-slate-600"
      }`}
    >
      <span
...
const SettingsScreen: React.FC = () => {
  ...
  const isHost =
    gameState.players.find((p) => p.id === playerId)?.isHost ?? false;
  ...
  return (
    <div className="animate-fadeIn max-w-2xl mx-auto space-y-6">
      <Card>
        <h2 className="font-eagleLake text-3xl mb-6 text-center text-yellow-500">
          Settings
        </h2>
        <div className="space-y-3">
          <Toggle
            label="Skip Intro Story"
            enabled={settings.skipIntro}
            onToggle={() => updateSettings({ skipIntro: !settings.skipIntro })}
          />
          <Toggle
            label="Stay Logged In"
            enabled={settings.stayLoggedIn}
            onToggle={() =>
              updateSettings({ stayLoggedIn: !settings.stayLoggedIn })
            }
          />
          <Toggle
        ...
        <div className="mt-6 border-t-2 border-slate-700 pt-4">
          <h3 className="font-eagleLake text-xl mb-4 text-center text-yellow-500">
            Change Username
          </h3>
          ...
          {isGameInProgress ? (
            <p className="text-amber-500 text-xs mt-2 text-center">
              Cannot change username while a game is in progress.
            </p>
          ) : (
            <>
          ...
          <div className="mt-6 border-t-2 border-slate-700 pt-4 space-y-4">
            <div>
              <Button
                ...
                disabled={onCooldown || !!gameState.restartVote}
                className="w-full"
              >
                {onCooldown
                  ? `Restart on Cooldown (${Math.ceil(cooldownTime / 1000)}s)`
            ...
            <div className="border-t-2 border-slate-700 pt-4">
              <h3 className="font-eagleLake text-xl mb-2 text-center text-red-500">
                Danger Zone
              </h3>
              <p className="text-slate-400 text-center text-xs mb-4">
                Kicking a player will immediately end the current game for
                everyone.
              </p>
