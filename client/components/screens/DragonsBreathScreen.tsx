...
// --- Card Data & Assets ---
const cardInfoMap = {
  [DragonCardType.DRAGON_BREATH]: {
    icon: Flame,
    borderColor: "border-red-500/80",
    name: "Dragon's Breath",
    description: "If you draw this and cannot defuse it, you lose.",
    image: "/cards/dragon_breath.png",
  },
  [DragonCardType.DEFUSE]: {
    ...
    name: "Defuse",
    description:
      "Play this to survive the Dragon's Breath. You can then re-insert the Dragon's Breath card anywhere in the deck.",
    image: "/cards/defuse.png",
  },
  [DragonCardType.ATTACK]: {
    ...
    name: "Attack",
    description:
      "End your turn without drawing. Your opponent must take two turns in a row.",
    image: "/cards/attack.png",
  },
  [DragonCardType.SKIP]: {
    icon: SkipForward,
  ...
  [DragonCardType.SEE_THE_FUTURE]: {
    ...
    image: "/cards/see_the_future.png",
  },
  ...
  [DragonCardType.EMBERDRAKE_HATCHLING]: {
    ...
    name: "Emberdrake Hatchling",
    description:
      "A cute but ultimately useless creature. Does nothing on its own.",
    image: "/cards/cat_1.png",
  },
  [DragonCardType.GLIMMERING_WHELP]: {
    icon: Heart,
    borderColor: "border-slate-500/80",
    name: "Glimmering Whelp",
    description:
      "A cute but ultimately useless creature. Does nothing on its own.",
    image: "/cards/cat_2.png",
  },
    ...
    >
      ...
      <AnimatePresence>
        {showInfo && (
          ...
          >
            <h3 className="font-eaglelake text-sm leading-tight mb-1 text-yellow-400">
              {info.name}
            </h3>
...
const CardBack: React.FC<{ className?: string; isClickable?: boolean }> = ({
  className,
  isClickable,
}) => (
  <div
...
const DragonsBreathScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (gameState.gameLog.length > prevGameLogLength.current) {
      ...
      newEntries.forEach((entry) => {
        ...
        if (
          entry.type === "dragonsBreath" &&
          !entry.text.startsWith(localPlayer.name)
        ) {
  ...
  // --- Modals ---
  const SeeTheFutureModal: React.FC<{
    cards: DragonCard[];
    onClose: () => void;
  }> = ({ cards, onClose }) => (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4 animate-fadeIn">
      <h2
        className="text-4xl font-eaglelake text-yellow-400 mb-2"
        style={{ textShadow: "0 0 10px rgba(234, 179, 8, 0.5)" }}
      >
        The Future...
      </h2>
      <p className="text-slate-300 mb-6">
        These are the top 3 cards of the deck.
      </p>
      <div className="flex flex-wrap justify-center gap-4 mb-8">
        {cards.length > 0 ? (
          cards.map((card, index) => (
            <PlayingCard key={card.id} card={card} numberOverlay={index + 1} />
          ))
        ) : (
          <p className="text-slate-400">The deck is empty.</p>
        )}
      </div>
  ...
  const PlaceDragonModal: React.FC<{
    deckSize: number;
    ...
    return (
      <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4 animate-fadeIn">
        <h2
          className="text-4xl font-eaglelake text-red-500 mb-2"
          style={{ textShadow: "0 0 10px rgba(220, 38, 38, 0.5)" }}
        >
          Place the Dragon's Breath
        </h2>
        <p className="text-slate-300 mb-6 max-w-sm text-center">
          You have defused the Dragon's Breath! Now, secretly place it back into
          the draw pile.
        </p>
        <div className="w-full max-w-sm bg-slate-900/50 p-6 rounded-2xl border border-slate-700 space-y-4">
          <Button onClick={() => onPlace(0)} className="w-full">
            Top of Deck (Position 0)
          </Button>
          <Button onClick={() => onPlace(deckSize)} className="w-full">
            Bottom of Deck (Position {deckSize})
          </Button>
          ...
          <div className="text-center pt-4 border-t border-slate-700">
            ...
            />
            <Button onClick={() => onPlace(index)} className="mt-4 w-full">
              Place at position {index}
            </Button>
          </div>
        </div>
      </div>
  ...
  const GameOverModal: React.FC<{ winner: Player; onLeave: () => void }> = ({
    winner,
    onLeave,
  }) => {
    useEffect(() => {
    ...
    return (
      <div className="fixed inset-0 bg-black/90 backdrop-blur-lg z-50 flex flex-col items-center justify-center p-4 animate-fadeIn">
        ...
        <h2 className="text-5xl font-eaglelake text-yellow-400 mb-4">
          Game Over!
        </h2>
        <p className="text-2xl text-white mb-4">
          <span className="font-bold">{winner.name}</span> is victorious!
        </p>
  ...
  const isUIBlocked =
    !!dbState.isViewingFuture || !!dbState.isPlacingDragon || !!dbState.winner;
  ...
  const handleCardClick = (card: DragonCard) => {
    ...
    if (isPlayable) {
      ...
      toast.info(
        "Defuse cards are used automatically when you draw the Dragon's Breath."
      );
    }
  };
  ...
  if (dbState.isViewingFuture === playerId)
    return (
      <SeeTheFutureModal cards={dbState.futureCards} onClose={endFutureView} />
    );
  if (dbState.isPlacingDragon === playerId)
    return (
      <PlaceDragonModal
        deckSize={dbState.deck.length}
        onPlace={placeDragonCard}
      />
    );
      ...
      <div className="w-full flex flex-col items-center gap-4 z-10">
        <div className="flex items-center gap-3 bg-black/40 backdrop-blur-sm p-2 px-4 rounded-lg border border-slate-700/50">
          <User
            ...
            className={cn(
              "transition-colors",
              !isMyTurn ? "text-yellow-400" : "text-slate-400"
            )}
          />
          <h2
            className={cn(
              "text-xl font-bold transition-all",
              !isMyTurn ? "text-yellow-400 animate-glow" : "text-slate-400"
            )}
          >
            {opponent.name}
          </h2>
      ...
      <div className="w-full max-w-lg mx-auto flex-grow flex flex-col items-center justify-center z-10 p-4 sm:p-6 my-4 bg-slate-900/50 backdrop-blur-md rounded-2xl border border-slate-700 relative">
        <div className="flex items-end justify-center gap-8 sm:gap-12 w-full mt-16">
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="relative w-36 sm:w-40 aspect-[3/4] rounded-xl border-2 border-slate-600/50">
              <AnimatePresence>
                {dbState.discardPile.length > 0 && (
                  ...
                  >
                    <PlayingCard
                      card={dbState.discardPile[dbState.discardPile.length - 1]}
                    />
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <span className="font-eaglelake text-base text-slate-300">
              Discard ({dbState.discardPile.length})
            </span>
          </div>
          {/* Draw Pile */}
          <div className="flex flex-col items-center gap-2">
            <button
              ...
              disabled={!isMyTurn || !!isUIBlocked}
              className="disabled:cursor-not-allowed"
            >
              <div>
                <CardBack
                  isClickable={isMyTurn && !isUIBlocked}
                  className={cn(
                    isMyTurn &&
                      !isUIBlocked &&
                      "animate-pulse-glow shadow-glow-yellow"
                  )}
                />
              </div>
            </button>
            <span className="font-eaglelake text-base text-slate-300">
              Draw Pile ({dbState.deck.length})
            </span>
          </div>
        </div>
      </div>
        ...
        >
          <AnimatePresence>
            {myHand.map((card, index) => (
              <motion.div
                ...
                layoutId={card.id}
                whileHover={
                  isHandExpanded ? { y: -20, scale: 1.05 } : { y: -8 }
                }
                className={cn(
                  !isHandExpanded && "absolute pointer-events-none"
                )}
                animate={{
                  rotate: isHandExpanded
                    ? 0
                    : (index - (myHand.length - 1) / 2) * 4,
                  zIndex: index,
                }}
                transition={{ type: "spring", stiffness: 400, damping: 40 }}
              >
          ...
          {!isHandExpanded && myHand.length > 0 && (
            <div className="absolute bottom-4 w-48 text-center p-2 bg-black/30 rounded-t-lg pointer-events-none">
              <p className="font-eaglelake text-yellow-400">
