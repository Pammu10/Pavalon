        ...
        <div className="bg-slate-900/50 p-2">
            <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row md:justify-between md:items-center text-center text-sm">
                <div className="mb-1 md:mb-0">
                    ...
                    <span className={`font-cinzel ml-2 font-bold ${alignmentColor}`}>({player.role})</span>
                </div>
                <div className="text-slate-300 italic">
                    <span className="font-semibold text-yellow-500 not-italic mr-1">Vision:</span>
                    {visiblePlayers.length > 0 ? visiblePlayers.map(p => p.name).join(', ') : 'You see nothing out of the ordinary.'}
                </div>
            </div>
        </div>
    );
