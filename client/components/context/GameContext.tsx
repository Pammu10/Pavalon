...
export const GameProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const [viewedSessionKeys, setViewedSessionKeys] = useState<Set<string>>(
    new Set()
  );
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        ...
        if (
          key &&
          (key.startsWith("viewedQuest-") || key.startsWith("viewedEndGame-"))
        ) {
          keys.add(key);
        }
      }
  ...
  const updateSettings = (newSettings: Partial<Settings>) => {
    setSettings((prev) => {
      ...
      // Handle session migration if stayLoggedIn changed
      if (
        isAuthenticated &&
        newSettings.stayLoggedIn !== undefined &&
        newSettings.stayLoggedIn !== prev.stayLoggedIn
      ) {
        const tokenToMove =
        ...
        if (tokenToMove && userToMove) {
          const targetStorage = newSettings.stayLoggedIn
            ? localStorage
            : sessionStorage;
          targetStorage.setItem("authToken", tokenToMove);
          targetStorage.setItem("user", userToMove);
          toast.success(
            newSettings.stayLoggedIn
              ? "You will now be kept logged in."
  ...
  const updateUser = (data: Partial<User>) => {
    setUser((prev) => {
      ...
      // Update the same storage the user is currently in
  ...
  useEffect(() => {
    ...
    // If not found, check sessionStorage for session-only login
  ...
  const handleUpdate = useCallback((newState: GameState) => {
    setGameState((prevState) => {
      ...
      if (
        ...
        prevState.phase === GamePhase.HOME
      ) {
        setJustJoined(true);
      }
      ...
      const isNewGameStarting =
        (prevState.phase === GamePhase.END_GAME &&
          newState.phase === GamePhase.LOBBY) ||
        (prevState.phase === GamePhase.LOBBY &&
          newState.phase === GamePhase.ROLE_REVEAL);
        ...
        if (prevState.roomCode) {
          ...
          for (let i = 0; i < sessionStorage.length; i++) {
            ...
            if (
              ...
              key.includes(prevState.roomCode)
            ) {
  ...
  const handleAchievementUnlocked = useCallback(
    (achievement: Achievement) => {
      ...
      toast.success(`Achievement Unlocked: ${achievement.name}`, {
        description: `You've earned: ${achievement.rewards
          .map((r) => r.name)
          .join(", ")}`,
        duration: 8000,
      });
  ...
  const login = async (
    ...
    try {
      ...
      const storage = settings.stayLoggedIn ? localStorage : sessionStorage;
      storage.setItem("authToken", new_token);
      storage.setItem("user", JSON.stringify(new_user));
  ...
  const placeDragonCard = (index: number) =>
  ...
  const hasViewedCurrentQuestResult = viewedSessionKeys.has(
    `viewedQuest-${gameState.roomCode}-${gameState.currentQuest}`
  );
  const hasViewedEndGameResult = viewedSessionKeys.has(
    `viewedEndGame-${gameState.roomCode}`
  );
