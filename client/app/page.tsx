...
const ReconnectionBanner: React.FC<{ player: { name: string, endsAt: number } }> = ({ player }) => {
    const [timeLeft, setTimeLeft] = useState(Math.round((player.endsAt - Date.now()) / 1000));
    ...
    return (
        <div className="fixed top-0 left-0 w-full bg-yellow-600/90 text-white font-bold text-center p-2 z-[200] flex items-center justify-center gap-4 animate-fadeIn">
            <Spinner size="sm" />
            <span>Waiting for {player.name} to reconnect... {timeLeft}s</span>
        </div>
    );
};

const MainContent: React.FC = () => {
    const { gameState, isAuthenticated, user } = useGame();
    ...
    const renderGameScreen = () => {
        switch (gameState.phase) {
            ...
            default: // HOME phase is now the lobby join/host screen for logged in users
                return <LobbyScreen />;
        }
    };
    ...
    const showPlayerInfo = gameState.phase !== GamePhase.HOME && gameState.phase !== GamePhase.LOBBY && gameState.phase !== GamePhase.END_GAME && gameState.phase !== GamePhase.ROLE_REVEAL;

    return (
        <div className="flex flex-col h-screen w-screen">
            {gameState.reconnectingPlayer && <ReconnectionBanner player={gameState.reconnectingPlayer} />}
                ...
                <nav className="flex justify-center">
                    {(['game', 'chat', 'stats'] as Tab[]).map((tab) => (
                        <button
                            ...
                            className={`flex-1 md:flex-none md:px-8 py-3 text-center font-cinzel text-lg capitalize transition-colors duration-200 ${activeTab === tab ? 'text-yellow-500 border-b-2 border-yellow-500' : 'text-slate-400 hover:text-white'}`}
                        >
                            {tab}
                        </button>
                    ))}
